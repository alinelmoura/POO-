...
import { UserNotRemoveError } from "./errors/user-not-remove-error";
...
export class App {
    ...
    async removeUser(email: string): Promise<void> {
        ...
        const rents=findOpenRentsFor(email)
        if(rents===RentNotFoundError()){
            await this.userRepo.remove(email)
        }
        else throw new UserNotRemoveError()
    }
